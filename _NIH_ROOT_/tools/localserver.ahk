...
SystemDrive:=SubStr(A_WinDir,1,2)
RegRead command,HKEY_CLASSES_ROOT\localserver\shell\open\command
RegRead URLProtocol,HKEY_CLASSES_ROOT\localserver,URL Protocol
URLProtocol:=ErrorLevel
If((FullPath:=(Path:=SystemDrive "\mickey90427\localserver\")"localserver.ahk")!=A_ScriptFullPath)
{
	...
	Loop
	{
		FileCopy % A_ScriptFullPath,% FullPath,1
...
If(command!="""%SystemDrive%\mickey90427\localserver\"RegExReplace(A_AhkPath,"^.+[\\/](.*)$","$1")""" ""%SystemDrive%\mickey90427\localserver\localserver.ahk""")||URLProtocol
{
	IfEqual A_IsAdmin,0,Run *RunAs "%A_ScriptFullPath%",,UseErrorLevel
	IfEqual A_IsAdmin,0,ExitApp
	RegWrite REG_SZ,HKEY_CLASSES_ROOT\localserver,URL Protocol
